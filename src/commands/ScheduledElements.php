...
class ScheduledElements extends Controller
{
    ...
    protected function drawTable(array $entries)
    {
        ...
        foreach ($entries as $entry) {
            ...
            $rows[]           = [$entry->title, $postDateString, $expiryDateString, $entry->id, $entry->slug];

            // Also write to stdout for cloud logging
            Craft::warning("K8 CronJob revalidating: $entry->slug", "REVALIDATE_STATUS");
        };

        echo Table::widget([
            'headers' => ['Title', 'PostDate', 'ExpiryDate', 'ID', 'Slug'],
            'rows'    => $rows,
        ]);
    }
